...
<head>
  ...
  <style>
    ...
    .table-container { max-height: 620px; overflow-y: auto; position: relative; }
...
<body class="bg-gray-50 min-h-screen flex flex-col">
  ...
  <div id="forwardModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
      ...
      <div class="space-y-4">
        <button class="w-full bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition-colors" onclick="forwardToSpecificUser()">
  ...
  <script>
    ...
    function loadEmails(deptId, pageToken = null) {
      ...
      fetch('./api/emails.php?' + params)
        ...
        .then(data => {
          ...
          // If not an infinite scroll load, clear the table first.
          if (!pageToken) {
            tableBody.innerHTML = '';
          }
    ...
    // Modal functions
    function openModal() { document.getElementById('forwardModal').classList.remove('hidden'); }
    function closeModal() { document.getElementById('forwardModal').classList.add('hidden'); }
    function forwardToSpecificUser() {
      const selectedEmails = getSelectedEmails();
      if (selectedEmails.length === 0) { alert('Please select at least one email to forward'); return; }
      alert(`Forwarding ${selectedEmails.length} email(s) to a specific user...`);
      closeModal();
    }
